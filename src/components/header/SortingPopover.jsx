...
const SortingPopover = ({ part }) => {
  ...
  const [sortIndex, setSortIndex] = useState(
    getCurrentIndexFromState(part, sortSettings)
  );
  ...
  function handleClickSet() {
    ...
    if (part === "anomaly") {
      newSortSettings = {
        ...sortSettings,
        anomaly: sortingOptionsModel[sortIndex - 1],
      };
    } else if (part === "face") {
      newSortSettings = {
        ...sortSettings,
        face: sortingOptionsModel[sortIndex - 1],
      };
    } else if (part === "plate") {
      newSortSettings = {
        ...sortSettings,
        plate: sortingOptionsModel[sortIndex - 1],
      };
    } else if (part === "report") {
      newSortSettings = {
        ...sortSettings,
        report: sortingOptionsModel[sortIndex - 1],
      };
    }
    ...
    <Popover
      ...
      content={
        <div className="flex flex-col">
          <p className="font-semibold">{`${textHere(
            TTForms.title_sort_by,
            lang
          )}:`}</p>
          <Radio.Group
            className="p-5"
          ...
          >
            <Space direction="vertical">
              {sortingOptionsModel.map((item, index) => (
                <Radio value={item.index} key={index}>
                  {item.label}
                </Radio>
              ))}
            </Space>
          </Radio.Group>
          <Button>{textHere(TTButtons.action_settings, lang)}</Button>
        </div>
      }
    >
