...
import { textHere, TTButtons, TTControlFace } from '../../../utils/intl';
...
// C step
const Control_Face_Images_C = ({ onFinish }) => {
    ...
    const uploadButton = (
        <div>
            ...
            >
                {textHere(TTControlFace.upload_face_file,lang)}
            </div>
        </div>
    );
    ...
    function handleUploadImage() {
        setFileList((prev) => [...prev, {
            uid: Date.now(),
            percent: 100,
            name: `image${currentImageCount}.png`,
            status: 'done',
            url: picture,
        }])
    ...
    function validateInputs() {
        return Object.keys(fileList).length > 0
    ...
    function onFinishHandler() {
        if(validateInputs()){
            setFaceData(prev=>({...prev, image:fileList}))
        onFinish(fileList, valid)

        }else{
            messageHandler('error',textHere(TTControlFace.alert_minimum_file_required,lang))
        }
    }
...
export default Control_Face_Images_C;
